...
class GASClient:
    ...
    def create_only(self,sheet:str,name:str,payload:Dict[str,Any], existing:Optional[List[str]]=None)->Tuple[bool,str]:
        ex = existing if existing is not None else self.list_names(sheet)
        if name in ex:
...
def template_block(
    ...
    with c2:
        new_name = st.text_input('另存為模板名稱', placeholder='例如：常用A（留空=更新目前套用模板）', key=new_key)
        save_btn = st.button('💾 儲存模板', use_container_width=True, key=f'{key_prefix}_save')

    with c3:
        del_sel = st.selectbox('要刪除的模板', names, key=del_key)
        del_btn = st.button('🗑️ 刪除模板', use_container_width=True, key=f'{key_prefix}_del')

    st.caption(f"目前套用：{active_now or '未選擇'}")
    ...
    if save_btn:
        ...
        if nm:
            ok,msg = gas.create_only(sheet, nm, payload, existing=tpl_list)
            if ok:
            ...
            if not active_now:
                ...
                ok,msg = gas.upsert(sheet, active_now, payload)
                if ok:
                    st.success(f'{msg}：{active_now}')
                    st.rerun()
                else:
                    st.error(msg)
...
def _item_pos_xyz(it:Item)->Tuple[float,float,float]:
...
def _muted_palette():
    return [
        ...
        "#556B2F", "#6B705C", "#7C6F64", "#5C4B51",
    ]

def build_3d_fig(box:Dict[str,Any], fitted:List[Item])->Tuple[go.Figure, Dict[str,str]]:
    ...
    fig.update_layout(
        scene=dict(
            xaxis=dict(title='長(L)', range=[0,L], backgroundcolor="rgba(0,0,0,0)"),
            yaxis=dict(title='寬(W)', range=[0,W], backgroundcolor="rgba(0,0,0,0)"),
            zaxis=dict(title='高(H)', range=[0,H], backgroundcolor="rgba(0,0,0,0)"),
            aspectmode='data'
        ),
...
def _legend_html(color_map:Dict[str,str])->str:
    if not color_map:
        return ""
    chips=[]
    for name, c in color_map.items():
        chips.append(
            ...
            f"<span style='width:12px;height:12px;border-radius:3px;background:{c};display:inline-block;margin-right:6px;'></span>"
            f"<span>{name}</span></span>"
        )
    return "<div style='margin-top:10px;color:#333;'><b>顏色對應：</b><div style='margin-top:6px;'>" + "".join(chips) + "</div></div>"

def build_report_html(order_name:str, box:Dict[str,Any], fitted:List[Item], unfitted:List[Item],
                      content_wt:float, total_wt:float, util:float, fig:go.Figure,
    ...
    if unfitted:
        ...
        warn="<div class='warn'><b>注意：</b>有部分商品裝不下！（可能是箱型庫存不足或尺寸不夠）</div>" + \
             ''.join([f"<div class='warn2'>⚠ {k}：超過 {v} 個</div>" for k,v in counts.items()])
    else:
        warn="<div class='ok'>✅ 裝箱成功：全部商品已放入外箱</div>"
...
.ok{{margin-top:12px;border:1px solid #b7ebc6;background:#ecfdf3;padding:10px 12px;border-radius:10px}}
</style></head><body>
<div class='container'>
  <div class='card'>
    <h1>📦 訂單裝箱報告</h1>
    <div class='row'>
      <div class='col'>
        ...
        <div class='kv'><div class='k'>使用外箱</div><div class='v'>{box['name']}（{box['l']}×{box['w']}×{box['h']}）× 1 箱</div></div>
      </div>
      <div class='col'>
...
def pack_and_render(order_name:str, df_box:pd.DataFrame, df_prod:pd.DataFrame)->Dict[str,Any]:
    ...
    used_vol=sum(_to_float(getattr(it,'width',0))*_to_float(getattr(it,'height',0))*_to_float(getattr(it,'depth',0)) for it in fitted)
    box_vol=float(box['l']*box['w']*box['h'])
    util=(used_vol/box_vol*100.0) if box_vol>0 else 0.0

    fig, color_map = build_3d_fig(box,fitted)
    html=build_report_html(order_name,box,fitted,unfitted,content_wt,total_wt,util,fig,color_map)
    return {
        'ok':True,
        'box':box,
        'fitted':fitted,
...
def result_block():
    ...
    if color_map:
        chips=[]
        for name,c in color_map.items():
            chips.append(f"<span style='display:inline-flex;align-items:center;margin-right:12px;margin-bottom:6px;'>"
                         f"<span style='width:12px;height:12px;border-radius:3px;background:{c};display:inline-block;margin-right:6px;'></span>"
                         f"<span>{name}</span></span>")
        st.markdown("<div class='muted'><b>顏色對應：</b><div class='small-gap'></div>" + "".join(chips) + "</div>", unsafe_allow_html=True)
