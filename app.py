...
<style>
    ...
    [data-testid="stSidebar"], [data-testid="stDecoration"], .stDeployButton, footer, #MainMenu { display: none !important; }
...
# ==========================
if run_button:
    with st.spinner('正在進行全邏輯演算...'):
        
        # 1. 全域變數初始化 (Single Source of Truth)
        # 這是唯一的真理帳本，所有邏輯最後都必須更新它
        ledger_final_count = {} 
        ...
        for index, row in edited_df.iterrows():
            try:
                ...
                if qty > 0:
                    # 登記需求
                    ledger_request_count[name] = ledger_request_count.get(name, 0) + qty
                    ...
                    if mode == "L型彎折 (作為內襯墊底)":
                        # 直接記入完成帳本 (因為我們會強制畫上去)
                        ledger_final_count[name] = ledger_final_count.get(name, 0) + qty
                    ...
                    # [Case B] 進入 Packer (攤平 / 對折)
                        ...
                        if "對折" in mode:
                            p_l, p_h = l/2, h*2
                            # 對折物品雖然變厚，但我們希望它不要佔據中間
                            # 這裡不改名稱，保持原名，方便統計
                        
                        # 決定優先級 (數值越小越先放)
                        # 規則：極薄(紙袋) = 0, 對折 = 5, 普通禮盒 = 10
                        priority = 10
                        if p_h < 1.0: priority = 0 # 攤平紙袋優先鋪底
                        elif "對折" in mode: priority = 5
                        
                        for _ in range(qty):
                            packer_items.append({
                                'obj': Item(name, p_l, w, p_h, weight), # Name 保持原名
                                'priority': priority,
                                'is_folded': "對折" in mode
                            })
        ...
        # 4. 關鍵排序與裝箱
        # 依照優先級排序：薄紙袋(0) -> 對折(5) -> 禮盒(10)
        packer_items.sort(key=lambda x: x['priority'])
        
        for p_item in packer_items:
            packer.add_item(p_item['obj'])
            
        # 執行裝箱 (False = 嚴格遵守我們的排序)
        ...
        # 顏色
        unique_names = list(ledger_request_count.keys())
        ...
        if lining_data:
            ...
            for i in range(lqty):
                ...
                fig.add_trace(go.Mesh3d(
                    ...
                    z=[z_pos, z_pos, z_pos, z_pos, z_pos+uh, z_pos+uh, z_pos+uh, z_pos+uh],
                ...
                fig.add_trace(go.Scatter3d(
                    ...
                    z=[z_pos, z_pos, z_pos, z_pos, z_pos, z_pos+uh, z_pos+uh, z_pos+uh, z_pos+uh, z_pos+uh, z_pos, z_pos+uh, z_pos+uh, z_pos+uh, z_pos, z_pos],
                ...
                x_pos = i * uh
                fig.add_trace(go.Mesh3d(
                    x=[x_pos, x_pos+uh, x_pos+uh, x_pos, x_pos, x_pos+uh, x_pos+uh, x_pos],
                    y=[0, 0, lining_data['w'], lining_data['w'], 0, 0, lining_data['w'], lining_data['w']],
                ...
                fig.add_trace(go.Scatter3d(
                    x=[x_pos, x_pos+uh, x_pos+uh, x_pos, x_pos, x_pos, x_pos+uh, x_pos+uh, x_pos, x_pos, x_pos, x_pos, x_pos+uh, x_pos+uh, x_pos+uh, x_pos+uh],
                    y=[0, 0, lining_data['w'], lining_data['w'], 0, 0, 0, 0, lining_data['w'], lining_data['w'], 0, lining_data['w'], lining_data['w'], lining_data['w'], 0, 0],
        ...
        for b in packer.bins:
            for item in b.items:
                # 這裡最重要：Packer 算出來有幾個，帳本就加幾個
                # 因為我們在放入 packer 時已經使用了原始名稱 (name)，所以直接用
                base_name = item.name 
                ledger_final_count[base_name] = ledger_final_count.get(base_name, 0) + 1
        ...
        # Layout (全黑字體)
        axis_s = dict(titlefont=dict(color="black"), tickfont=dict(color="black"), backgroundcolor="white", gridcolor="#999999", showbackground=True)
        fig.update_layout(
            template="plotly_white", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
            font=dict(color="black"),
            scene=dict(xaxis={**axis_s, 'title':'長'}, yaxis={**axis_s, 'title':'寬'}, zaxis={**axis_s, 'title':'高'}, aspectmode='data', camera=dict(eye=dict(x=1.6, y=1.6, z=1.6))),
            margin=dict(t=30, b=0, l=0, r=0), height=600, legend=dict(x=0, y=1, bgcolor="rgba(255,255,255,0.8)", borderwidth=1, font=dict(color="black"))
        )

        # 5. 最終報表 (比對 ledger_request_count vs ledger_final_count)
        ...
        for name, req in ledger_request_count.items():
            real = ledger_final_count.get(name, 0)
            diff = req - real
            if diff > 0:
                all_fitted = False
